...
class _FakeGemini:
    ...
    def generate(self, **kwargs):
        ...
        return {'content': 'from gemini', 'model': 'gemini-1.5-flash',
                'provider': 'gemini', 'usage': {'input_tokens': 1, 'output_tokens': 2}}
...
def _make_chat_service(monkeypatch, provider, gemini):
    ...
    # Stub Ollama agent.
    monkeypatch.setattr(svc.agent, 'invoke_agent',
                        lambda **kwargs: {'response': 'from ollama', 'metadata': {'tool_calls': []}})
